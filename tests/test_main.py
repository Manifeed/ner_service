...
from app.schemas.ner_schema import NerBatchResponseRead
...
def test_entities_batch_route_returns_503_when_runtime_is_unavailable(monkeypatch) -> None:
	_patch_runtime_client(monkeypatch)
	monkeypatch.setattr(
...
def test_entities_batch_route_returns_401_with_invalid_api_key(monkeypatch) -> None:
	_patch_runtime_client(monkeypatch)
	monkeypatch.setenv("NER_SERVICE_API_KEY", "test-key")
	monkeypatch.setattr(
		"app.routers.ner_router.create_entities_batch",
		lambda payload: NerBatchResponseRead(data=[]),
	)
...
def test_entities_batch_route_returns_specific_service_errors(monkeypatch) -> None:
	_patch_runtime_client(monkeypatch)
	client = TestClient(app)
